...
class AniDBProvider(BaseProvider):
    ...
    async def get_source_offers(
        ...
        try:
            # 1. ID Mapping Layer: Check cache first
            best_match = self._mapping_cache.get(mapped_id)
            title = None

            if context:
                title = context.english_title or context.primary_title or context.romaji_title
            
            if not best_match:
                if not title:
                    if is_mal:
                        resp = await self.client.get(f"{self.JIKAN_API}/{raw_id}", headers=self.HEADERS)
                ...
                # Candidate titles to search against
                candidate_titles = [title]
                if context:
                    for syn in [context.primary_title, context.romaji_title, context.english_title] + context.synonyms:
                        if syn and syn not in candidate_titles:
                            candidate_titles.append(syn)

                for query in candidate_titles:
                    search_url = f"{self.BASE_URL}/browse?q={urllib.parse.quote(query)}"
                    search_resp = await self.client.get(search_url, headers=self.HEADERS)
                ...
                # Cache mapped show ID
                self._mapping_cache[mapped_id] = best_match

            # 2. Episode Listing Layer: Check cache first
            if best_match in self._episodes_cache:
                episodes = self._episodes_cache[best_match]
            else:
                ep_resp = await self.client.get(f"{self.BASE_URL}/api/frontend/anime/{best_match}/episodes", headers=self.HEADERS)
            ...
            payload_data = {"ep_id": internal_ep_id, "is_dub": is_dub}
            payload_id = encode_ep_id(payload_data)
            
            display_title = title or context.primary_title if context else mapped_id
            # media_id lets the player UI resolve the episode/season drawer via
            # /api/player/episodes/{media_id} — without it, initPlayerEpisodes()
            # in player.html has no id to fetch and the drawer never activates.
            external_player_url = (
    ...
    async def extract_stream(self, provider_ep_id: str) -> PlayerStateModel:
        ...
        lang_resp = await self.client.get(f"{self.BASE_URL}/api/frontend/episode/{internal_ep_id}/languages", headers=self.HEADERS)
        ...
        embed_resp = await self.client.get(embed_url, headers=self.HEADERS)
